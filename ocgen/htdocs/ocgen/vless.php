...
if(isset($_POST['save'])){
   ...
   // Membaca nama file yaml yang dipilih
    ...
    $yaml_content = file_get_contents("/etc/openclash/proxy_provider/" . $yaml_name . ".yaml");	
...
file_put_contents("/etc/openclash/proxy_provider/" . $yaml_name . ".yaml", $yaml_content);

// Tampilkan notifikasi sukses menyimpan config
			echo "<div class='alert alert-success'>";
			echo "<h4>YAML Configuration Saved!</h4>";
			echo "<p>File saved at <b>../etc/openclash/proxy_provider</b> as: <a href='/tinyfm/index.php?p=etc%2Fopenclash%2Fproxy_provider&edit=$yaml_name.yaml'>$yaml_name.yaml</a> name</p>";
			echo " </div>";
   ...
   <i class="fa fa-random" aria-hidden="true"></i> Vless WS Reverse
    ...
    <?php else: ?>
        ...
        <form method="post">
            ...
            $folder = "/etc/openclash/proxy_provider";
            ...
            ?>
                <!-- Tampilkan pilihan file di dalam folder -->
				  <fieldset id="servers">
				      <div class="server">
                <label for="yaml_name">Select YAML Config <a href='/tinyfm/index.php?p=etc%2Fopenclash%2Fproxy_provider'>(check here)</a> :</label>
